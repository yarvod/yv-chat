...
const statusLabel = computed(() => {
  switch (push.state.value.status) {
    ...
    case 'denied': return $frontend.platform.native
      ? 'Заблокированы в настройках устройства'
      : 'Заблокированы в настройках браузера'
    case 'unsupported': return $frontend.platform.native
...
<template>
  <article class="settings-card settings-card--wide notification-settings-card">
    ...
    <p
      v-if="push.state.value.status === 'unsupported' && !$frontend.platform.native"
