...
async def test_create_direct_group_list_and_non_member_get() -> None:
    ...
    direct = await CreateDirectConversation(unit_of_work=factory, clock=clock).execute(
        CreateDirectConversationCommand(alice.id, bob.id)
    )
    with pytest.raises(DuplicateDirectConversationError):
        await CreateDirectConversation(unit_of_work=factory, clock=clock).execute(
            CreateDirectConversationCommand(bob.id, alice.id)
        )
    group = await CreateGroupConversation(unit_of_work=factory, clock=clock).execute(
        CreateGroupConversationCommand(alice.id, "MVP team", (bob.id,))
    )
...
async def test_group_role_policy_add_remove_and_removed_member_visibility() -> None:
    ...
    with pytest.raises(AuthorizationDeniedError):
        await AddConversationMember(unit_of_work=factory, clock=clock).execute(
    ...
    with pytest.raises(AuthorizationDeniedError):
        await RemoveConversationMember(unit_of_work=factory, clock=clock).execute(
            RemoveConversationMemberCommand(bob.id, group.conversation_id, alice.id)
        )

    await RemoveConversationMember(unit_of_work=factory, clock=clock).execute(
        RemoveConversationMemberCommand(alice.id, group.conversation_id, bob.id)
    )
    with pytest.raises(ConversationNotFoundError):
        await GetConversation(unit_of_work=factory).execute(
...
async def test_member_can_leave_but_owner_cannot() -> None:
    ...
    with pytest.raises(AuthorizationDeniedError):
        await LeaveConversation(unit_of_work=factory, clock=clock).execute(
